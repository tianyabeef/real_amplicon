...
def get_html(cfg_in, vars=None):
    ...
    command.append(
            'wkhtmltopdf --margin-right 5mm --margin-left 5mm  --footer-right \'--第[page]页--\' --header-line --footer-line --header-center 上海锐翌生物科技有限公司 %s %s \n' % (
                out_dir_report + '/pdf.html', out_dir_report + '/report.pdf'))
    #  command.append('rm %s/pdf.html\n'%out_dir_report) param --exclude-from-outline
