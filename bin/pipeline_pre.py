...
if __name__ == '__main__':
    work_cfg = PWD + '/work_pre.cfg'
    cf = ConfigParser.RawConfigParser()
    cf.read(work_cfg)
    group_files_xlsx = cf.get("params","group_files_ori")
    os.popen('/data_center_01/pipeline/16S_ITS_pipeline_v3.0/script/parseXml.py -i %s -o %s/group/' % (group_files_xlsx,PWD))
    list_res = []
    for value in os.popen('ls %s/group/' % PWD ).readlines():
        if re.search("VS",value.strip()):
            list_res.append('%s/group/%s' % (PWD,value.strip()))
    cf.set('params','group_files',' '.join(list_res))
    cf.set('params','alpha_group_file','%s/group/alpha_group.txt' % PWD)
    cf.set('params','name_list','%s/group/newname.list' % PWD)
    cf.write(open(work_cfg, "w"))
