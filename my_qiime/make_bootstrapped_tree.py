...
from bootstrapped_tree import write_pdf_bootstrap_tree
...
def main():
    ...
    color_dict = {}
    with open(opts.support) as fp:
        for line in fp:
            tabs = line.rstrip().split('\t')
            color_dict[tabs[0]] = tabs[1]

    # support_file.close()
    write_pdf_bootstrap_tree(tree, opts.output_file, color_dict)
