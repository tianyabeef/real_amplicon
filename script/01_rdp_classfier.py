...
def read_params(args):
    parser = argparse.ArgumentParser(description='classify seq using rdp | v1.0 at 2015/09/18 by liangzb')
    parser.add_grgument('-j','--jar',dest='jar',metavar='JAR',type=str,required=True,
    ...
    parser.add_argument('-h','--hier_outfile',dest='hierfile',metavar='FILE',type=str,default=None
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(params['outfile']):
        os.mkdir(os.path.dirname(params['outfile']))
    if params['hierfile'] and not os.path.isdir(params['hierfile']):
    ...
    commads += ' -c %s -o %s'%(params['conf'],params['outfile'])
    if params['hierfile']:
