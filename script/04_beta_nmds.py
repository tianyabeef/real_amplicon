...
def read_params(args):
    parser = argparse.ArgumentParser(description='''beta NMDS | v1.0 at 2015/10/16 by liangzb ''')
...
def work(r_job, name, params):
    ...
    pdf_file = '%s/%s.nmds.pdf' % (params['out_dir'], name)
    png_file = '%s/%s.nmds.png' % (params['out_dir'], name)
    R_file = '%s/%s.nmds.R' % (params['out_dir'], name)
