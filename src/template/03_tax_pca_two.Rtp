...
if(nrow(X)<=1){
    ...
    p <-ggplot(X.dudi$li,aes(x=Axis1,y=Axis2,shape=group,colour=time))+geom_point(size=5)+scale_shape_manual(values = scale_shape_manual)+scale_color_manual(values = scale_color_manual)
